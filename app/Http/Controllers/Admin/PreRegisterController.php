...
class PreRegisterController extends Controller
{
    ...
    public function create(Request $request)
    {
        if(auth()->user()->getrole->name == 'Employee') {
            $this->data['employees'] = Employee::where(['status'=>Status::ACTIVE,'location_id'=>auth()->user()->employee->location_id])->get();
        }else {
    ...
    public function getPreRegister(Request $request)
    {
        ...
        return Datatables::of($pre_registerArray)
            ...
            ->editColumn('uid', function ($pre_register) {
                return Str::limit($pre_register->uid, 50);
            })
